...
func NewMsgSend(prefix string, from, to types.AccAddress, amount types.Coins) *banktypes.MsgSend {
	fromAddress, _ := types.Bech32ifyAddressBytes(prefix, from.Bytes())
	toAddress, _ := types.Bech32ifyAddressBytes(prefix, to.Bytes())
...
func (t CryptoTx) getUnsignedTxAndHash() (string, string, error) {
	encCfg := simapp.MakeTestEncodingConfig()
	txBuilder := encCfg.TxConfig.NewTxBuilder()

	from, err := types.GetFromBech32(t.From, t.AddrPrefix)
	if err != nil {
		return "", "", err
	}

	to, err := types.GetFromBech32(t.To, t.AddrPrefix)
	if err != nil {
		return "", "", err
	}

	//msg := banktypes.NewMsgSend(from, to, types.NewCoins(types.NewInt64Coin(t.Denom, t.Amount)))
	msg := NewMsgSend(t.AddrPrefix, from, to, types.NewCoins(types.NewInt64Coin(t.Denom, t.Amount)))
